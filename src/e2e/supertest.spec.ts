...
describe('given an "app" with "/treatments" route', () => {
    describe('when we connect with mongoDB', () => {
        ...
        test('then the get to urls /treatments should send a 200 status', async () => {
            await request(app).get('/treatments').expect(200);
        });

        test('then the get to urls /treatments/:id, if id is not in the DB, should send status 403', async () => {
            const response = await request(app).get(
                '/treatments/6378d483b74569e5d87e8685'
            );
            expect(response.status).toBe(403);
        });
        test('then the get to urls /treatments/:id, if id is bad formed, should send status 403', async () => {
            const response = await request(app).get('/treatments/34');
            expect(response.status).toBe(403);
        });
        test('then the post to urls /treatments without authorization should send status 403', async () => {
            const response = await request(app)
                .post('/treatments/create')
                .send({ name: 'aldana' });
            expect(response.status).toBe(403);
        });
        test('then the post to urls /treatments with authorization should send status 201', async () => {
            const response = await request(app)
                .post('/treatments/create')
        ...
        test('then the patch to urls /treatments/update/:id without authorization should send status 403', async () => {
            const response = await request(app)
        ...
        test('then the patch to urls /treatments/update/:id with authorization should send status 202', async () => {
            const response = await request(app)
        ...
        test('then the delete to urls /treatments/delete/:id with authorization should send status 200', async () => {
            const response = await request(app)
        ...
        test('then the delete to urls /treatments/delete/:id without authorization should send status 403', async () => {
            const response = await request(app)
                .delete(`/treatments/delete/${treatmentIDS[0]}`)
                .send({ name: 'marcos' });
            expect(response.status).toBe(403);
