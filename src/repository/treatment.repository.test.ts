...
describe('Given TreatmentRepository', () => {
    describe('When we instantiate it', () => {
        ...
        test('Then if id is null delete should throw an error', async () => {
            expect(async () => {
                await repository.delete('');
            }).rejects.toThrow();
        });
        test('Then if id is bad delete should throw an error', async () => {
            expect(async () => {
        ...
        test('Then if id is bad find should throw an error', async () => {
            expect(async () => {
                await repository.find({ id: '6378d483b738f8e5d87e8285' });
            }).rejects.toThrow();
        });
        test('Then if id is not an id find should throw an error', async () => {
            expect(async () => {
                await repository.find({ id: '23' });
            }).rejects.toThrow();
        });
        test('Then if id is null find should throw an error', async () => {
            expect(async () => {
